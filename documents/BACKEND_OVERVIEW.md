...
- 라이브 중인 종목에 대해 현재 라이브 세션의 배당 기준 수량 기준으로 1시간마다 배당을 지급한다.
...
| 배당 | 현재 라이브 세션의 배당 기준 수량(`preStreamQuantity`)에 대해 시간당 현재가의 0.01% |
...
| `stock` | 종목 FK. |
| `quantity` | 현재 보유 수량. |
| `preStreamQuantity` | 현재 라이브 세션의 배당 기준 수량. 라이브 시작 시 보유량으로 초기화되고, 라이브 중 매수분도 다음 배당부터 포함되도록 증가한다. |
| `avgPrice` | 평균 매입 단가. |
| `updatedAt` | 마지막 수정 시각. |

매도 시 `quantity`가 줄어들면 `preStreamQuantity`도 현재 보유량보다 클 수 없도록 함께 줄인다. 즉 보유량을 팔았는데도 과거 배당 기준 수량만으로 계속 배당을 받는 상황을 막는다.
...
배당은 현재 라이브 세션의 배당 기준 수량인 `preStreamQuantity`를 사용한다. 이 값은 라이브 시작 시 보유량으로 초기화되고, 라이브 중 매수분은 다음 배당부터 포함되도록 증가한다.
