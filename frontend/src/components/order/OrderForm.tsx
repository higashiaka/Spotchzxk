...
export const OrderForm = ({
  ...
  // 현재가 로드 시 지정가 초기값 설정
  useEffect(() => {
    if (limitPrice === null && currentPrice > 0) {
      setLimitPrice(currentPrice);
    }
  }, [currentPrice, limitPrice, setLimitPrice]);
  ...
  const activePrice = orderMode === 'limit' ? (limitPrice ?? currentPrice) : currentPrice;

  // 순수 호가창 매칭 시스템: 가격 영향(슬리피지) 공식 제거
  ...
  const maxBuy = activePrice > 0 ? Math.floor(balance / activePrice) : 0;
  const maxSell = held;
  ...
  const handleSubmit = () => {
    ...
    tradeMutation.mutate({
      ...
      orderMode,
      limitPrice: orderMode === 'limit' ? activePrice : undefined,
    });
  };
    ...
    <div className="h-full overflow-y-auto p-4 pb-24 hide-scrollbar bg-[#0B0E14] text-white">
      ...
      <div className="rounded-xl p-1 flex mb-4 bg-[#131924]">
        <button
          ...
          style={{
            backgroundColor: orderType === 'buy' ? '#FF5252' : 'transparent',
            color: orderType === 'buy' ? '#fff' : '#626B7A',
          }}
        >
          매수
        </button>
        <button
          ...
          className="flex-1 py-2 rounded-lg text-xs font-extrabold transition-all"
          style={{
            backgroundColor: orderType === 'sell' ? '#3D8BFF' : 'transparent',
            color: orderType === 'sell' ? '#fff' : '#626B7A',
          }}
        >
          매도
        </button>
      ...
      <div className="rounded-xl p-1 flex mb-4 bg-[#131924]">
        <button
          ...
          style={{
            backgroundColor: orderMode === 'market' ? '#222A3A' : 'transparent',
            color: orderMode === 'market' ? '#fff' : '#626B7A',
          }}
        >
          시장가
        </button>
        <button
          ...
          className="flex-1 py-1.5 rounded-lg text-xs font-bold transition-colors"
          style={{
            backgroundColor: orderMode === 'limit' ? '#222A3A' : 'transparent',
            color: orderMode === 'limit' ? '#fff' : '#626B7A',
          }}
        >
          지정가
        </button>
      ...
      {/* 지정가 입력 필드 (지정가 모드 시 표시) */}
      {orderMode === 'limit' && (
        <div className="mb-4">
          <p className="text-[10px] font-bold mb-1 text-[#8491A5]">지정 가격 (원)</p>
          <div className="relative">
            <input
              ...
              className="w-full rounded-xl border py-2.5 px-3 text-white font-mono text-base focus:outline-none focus:border-[#FF5252] bg-[#131924] border-[#222A3A]"
            />
            <span className="absolute right-3 top-2.5 text-xs text-[#626B7A]">원</span>
          </div>
        </div>
      )}
      ...
      {/* 단가 요약 */}
      <div className="mb-4">
        <p className="text-[10px] font-bold mb-1 text-[#8491A5]">
          {orderMode === 'limit' ? '지정 단가' : '현재가 (체결 기준)'}
        </p>
