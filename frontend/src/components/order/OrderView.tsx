...
export const OrderView = ({
  ...
  if (selectedStreamer) {
    return (
      <div className="h-full overflow-hidden" style={{ background: 'var(--bg-sidebar)' }}>
        <OrderForm
  ...
  return (
    <div className="h-full overflow-y-auto p-4 pb-24 hide-scrollbar" style={{ background: 'var(--bg-sidebar)' }}>
        ...
        {sorted.map(s => {
          ...
          return (
            <div key={s.id} onClick={() => onSelectStreamer(s)}
              className="flex items-center px-4 py-3 rounded-xl border cursor-pointer transition-colors hover:border-[#3D8BFF]" style={{ background: 'var(--bg-card-secondary)', borderColor: 'var(--border-primary)' }}>
