      ...
      {visiblePosts.map(post => {
        const content = (
          ...
          <div className="text-right shrink-0">
            ...
            {post.liveUrl && <p className="text-[10px]" style={{ color: '#00AAFF' }}>링크 보기 →</p>}
          </div>
          </>
        );
        const className = `flex items-center gap-3 rounded-xl transition-all hover:brightness-110 ${compact ? 'px-3 py-2.5' : 'px-4 py-3'}`;
        const style = { background: 'var(--bg-sidebar)', border: '1px solid #222A3A', textDecoration: 'none' };
        return post.liveUrl ? (
          <a key={post.id} href={post.liveUrl} target="_blank" rel="noopener noreferrer" className={className} style={style}>
            {content}
          </a>
        ) : (
          <div key={post.id} className={className} style={style}>
            {content}
          </div>
        );
      })}
