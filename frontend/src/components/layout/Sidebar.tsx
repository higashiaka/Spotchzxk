...
export const Sidebar = ({
  ...
  collapsed?: boolean;
  /** Desktop collapse toggle handler */
  ...
  return (
    <div
      className={`${activeTab === 'profile' ? 'flex' : 'hidden'} md:flex flex-col w-full ${isCollapsed ? 'md:w-16' : 'md:w-[300px]'} md:shrink-0 h-full overflow-hidden surface-sidebar border-r border-primary-token transition-[width] duration-200`}
    >
      {isCollapsed ? (
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {onToggleCollapsed && (
              <button
                type="button"
                onClick={onToggleCollapsed}
                className="hidden md:flex w-9 h-9 md:w-10 md:h-10 rounded-xl border items-center justify-center shrink-0 transition-colors hover:opacity-80 active:opacity-60"
                style={{
                  background: 'var(--bg-card)',
                  borderColor: 'var(--border-primary)',
                  color: 'var(--text-dim)',
